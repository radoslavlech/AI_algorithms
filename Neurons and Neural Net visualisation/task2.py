...
n1 = Neuron(data_train,'heaviside')
n1.learn(epochs=20)

n2 = Neuron(data_train,'sigmoid')
n2.learn(epochs=20)

n3 = Neuron(data_train,'sin')
n3.learn(epochs=20)

n4 = Neuron(data_train,'tanh')
n4.learn(epochs=20)

n5 = Neuron(data_train,'sgn')
n5.learn(epochs=20)

n6 = Neuron(data_train,'relu')
n6.learn(epochs=20)

n7 = Neuron(data_train,'leaky_relu')
n7.learn(epochs=20)

neuron_list = [n1,n2,n3,n4,n5,n6,n7]
for neuron in neuron_list:
    correct_count = 0
    for item in data_eval:
        label = item[-1]
        input = np.array(item[:2])
        a = neuron.classify(input)
        if (a > 0.5 and label ==1) or (a <0.5 and label==0):
            correct_count+=1
    print(f'Activation f.: {neuron.activation_function}. Accuracy: {correct_count/len(data_eval)*100}%')
...
app2.layout = html.Div(
    ...
    children=[
        ...
        html.Div(children=[dcc.Graph(id='graph2', figure=create_scatter_line(df2, n2))], style=graph_style),
        html.Div(children=[dcc.Graph(id='graph3', figure=create_scatter_line(df2, n3))], style=graph_style),
        html.Div(children=[dcc.Graph(id='graph4', figure=create_scatter_line(df2, n4))], style=graph_style),
        html.Div(children=[dcc.Graph(id='graph5', figure=create_scatter_line(df2, n5))], style=graph_style),
        html.Div(children=[dcc.Graph(id='graph6', figure=create_scatter_line(df2, n6))], style=graph_style),
        html.Div(children=[dcc.Graph(id='graph7', figure=create_scatter_line(df2, n7))], style=graph_style),
])])
